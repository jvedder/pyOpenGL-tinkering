...
def create_torus_data():
    ...
    # Generate quads (each quad gets its own 4 vertices to avoid color interpolation)
    for i in range(major_segments):
        for j in range(minor_segments):
            # Determine color for this quad
            color_index = int(i / (major_segments / 4)) % 4
            color = colors[color_index]
            
            # Create 4 vertices for this quad with the same color
            quad_vertices = []
            for di in range(2):  # 0, 1
                for dj in range(2):  # 0, 1
                    # Calculate angles
                    theta = 2.0 * np.pi * (i + di) / major_segments
                    phi = 2.0 * np.pi * (j + dj) / minor_segments
                    ...
                    quad_vertices.append(vertex_count)
                    vertex_count += 1
            
            # Create two triangles from the quad
            # Triangle 1: 0, 1, 2
            indices.extend([quad_vertices[0], quad_vertices[1], quad_vertices[2]])
            # Triangle 2: 1, 3, 2
            indices.extend([quad_vertices[1], quad_vertices[3], quad_vertices[2]])
